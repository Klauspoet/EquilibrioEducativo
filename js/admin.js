  ...
  try {
    ...
    psicos.forEach(psico => {
      ...
      const htmlBotonTitulo = url
        ? `<a href="${escapeHtml(url)}" target="_blank" class="btn-titulo">📄 Ver título</a>`
...
window.aprobar = async (id) => {
  try {
    ...
    if (error) {
      ...
      alert('No se pudo aprobar al psicoorientador. Intenta de nuevo.')
      return
    }
...
window.rechazar = async (id) => {
  try {
    ...
    if (error) {
      ...
      alert('No se pudo rechazar al psicoorientador. Intenta de nuevo.')
      return
    }
    cargarPsicoorientadores()
  } catch (err) {
