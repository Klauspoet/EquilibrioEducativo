// IMPORTANT: Supabase RLS must allow SELECT on "mensajes" for
// authenticated users who are part of the chat.
// Required policy in Supabase SQL editor:
// CREATE POLICY "usuarios pueden ver mensajes de sus chats"
// ON mensajes FOR SELECT
// USING (
//   auth.uid() IN (
//     SELECT estudiante_id FROM chats WHERE id = chat_id
//     UNION
//     SELECT psicoorientador_id FROM chats WHERE id = chat_id
//   )
// );
...
async function cargarInfoChat() {
  try {
    ...
    const avatarEl = document.getElementById('avatar-receptor')
    if (avatarEl && nombreReceptor) {
      avatarEl.textContent = nombreReceptor.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2)
    }
  } catch (err) {
  ...
  try {
    ...
    data.forEach(msg => {
      ...
      div.innerHTML = `
        ...
        <span class="hora">${new Date(msg.enviado_en).toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit', hour12: true })}</span>
      `
...
function agregarMensajeDOM(texto, nombre) {
  ...
  div.innerHTML = `
    ...
    <span class="hora">${new Date().toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit', hour12: true })}</span>
  `
  mensajesDiv.appendChild(div)
  mensajesDiv.scrollTop = mensajesDiv.scrollHeight
}
...
if (btnEnviar && inputMensaje) {
  btnEnviar.addEventListener('click', async () => {
    ...
    try {
      ...
      if (errorMensaje) {
        const lastMsg = mensajesDiv.querySelector('.mensaje:last-child')
...
async function iniciarSuscripcion() {
  suscripcion = supabase
    ...
    .on('postgres_changes', {
      ...
      if (payload.new.enviado_por === usuarioActual?.id) return
      ...
      div.innerHTML = `
        <span class="nombre">${escapeHtml(remitente?.nombre || 'Usuario')}</span>
        <p>${escapeHtml(payload.new.texto)}</p>
        <span class="hora">${new Date(payload.new.enviado_en).toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit', hour12: true })}</span>
      `
      mensajesDiv.appendChild(div)
      mensajesDiv.scrollTop = mensajesDiv.scrollHeight

      if (remitente) mostrarNotificacion(remitente.nombre, payload.new.texto)
    })
    .subscribe()
}
